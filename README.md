...
`Control` cell lines, the next step is to perform the *in silico*
genetic screen. This uses the dependency probabilities (or “lethality
probabilities”) generated from DepMap’s genome-wide CRISPR-Cas9 knockout
screen. Lethality probabilities range from 0.0 to 1.0 and is quantified
for each gene knock out in every cancer cell line screened (There are
18,334 genes targeted in 739 cancer cell lines). A gene knock out with a
...
# Visualize results, turn on gene labels, and label three genes each that are predicted to have lethal and alleviating genetic interactions, respectively
