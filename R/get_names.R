...
get_GeneNameID <- function(gene_name, data_dir) {
  ...
  if (any(dep_annot$GeneNames %in% gene_name | dep_annot$GeneID %in% gene_name)) {
    res <- dep_annot %>%
      dplyr::filter(.data$GeneNames %in% gene_name | .data$GeneID %in% gene_name) %>%
      dplyr::pull(.data$GeneNameID)
    
    # For Hugo IDs - from CCLE RNA-seq expr.
  } else if (any(CCLE_exp_annot$GeneNames %in% gene_name | CCLE_exp_annot$GeneID %in% gene_name)) {
    res <- CCLE_exp_annot %>%
      dplyr::filter(.data$GeneNames %in% gene_name | .data$GeneID %in% gene_name) %>%
      dplyr::pull(.data$GeneNameID)
    ...
    stop(
      "Cannot find gene name. Please check the spelling. Gene names should be a valid Hugo Symbol and in all caps!"
    )
  }
  return(res)
}
...
get_DepMapID <- function(sample_name, data_dir) {
  ...
  if (any(protein_annot$GygiNames %in% sample_name)) {
    ...
    stop(
      "Cannot find sample. Please check the spelling. Common sample names should be all caps without spaces or special symbols!"
    )
  }
  return(res)
}
