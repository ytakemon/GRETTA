...
get_inflection_points <- function(Input_coessential_df = NULL){
  ...
  fit_pos <- RootsExtremaInflections::inflexi(x_pos,y_pos,1,length(x_pos),5,5, plots = F, doparallel = F);fit_pos$an;fit_pos$finfl;
  ...
  fit_neg <- RootsExtremaInflections::inflexi(x_neg,y_neg,1,length(x_neg),5,5, plots = F, doparallel = F);fit_neg$an;fit_neg$finfl;
