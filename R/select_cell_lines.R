...
#' @param Input_disease string Cancer type selected from `list_available_cancer_types()`
...
select_cell_lines <- function(Input_gene = NULL, Input_disease = NULL, Input_disease_subtype = NULL){
  
  # Print and check to see input
  if(is.null(Input_gene) & is.null(Input_disease) & is.null(Input_disease_subtype)){
    stop("No input given. Please prvide an Hugo gene symbol and/or cancer type")
  } else if(is.null(Input_gene) & is.null(Input_disease) & !is.null(Input_disease_subtype)){
    stop("No cancer context provided. Please define the `disease` argument.")
  } else if(!is.null(Input_gene) & is.null(Input_disease) & is.null(Input_disease_subtype)){
    print(paste0("Selecting mutant groups for: ", Input_gene, " in all cancer cell lines"))
  } else if(!is.null(Input_gene) & !is.null(Input_disease) & !is.null(Input_disease_subtype)){
    print(paste0("Selecting mutant groups for: ", Input_gene, " in ", Input_disease,", ", Input_disease_subtype, " cell lines"))
  } else if(!is.null(Input_gene) & !is.null(Input_disease) & is.null(Input_disease_subtype)){
    print(paste0("Selecting mutant groups for: ", Input_gene, " in ", Input_disease, " cell lines"))
  } else if(is.null(Input_gene) & !is.null(Input_disease) & !is.null(Input_disease_subtype)){
    print(paste0("Selecting all ", Input_disease, ", ", Input_disease_subtype, " cancer cell lines"))
  } else if(is.null(Input_gene) & !is.null(Input_disease) & is.null(Input_disease_subtype)){
    print(paste0("Selecting all ", Input_disease, " cancer cell lines"))
  } else {
